...
public class BlockTile : MonoBehaviour
{
    ...
    public void OnHitCollision(ContactPoint2D contactPoint)
    {
        ...
        if (_currentHits >= _totalHits)
        {
            ...
            int _rand = Random.Range(0, _powerUps.Count -1);
            int _randProbability = Random.Range(0,3);
            Debug.Log(_randProbability);
            if(_randProbability == 1)
            {
                GameObject instance = Instantiate(_powerUps[_rand],transform.position,Quaternion.identity);
            }
